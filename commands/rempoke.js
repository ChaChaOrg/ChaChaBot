...
module.exports.run = (client, connection, P, message, args) => {
    try {
        ...
        console.log("Attempting to remove " + pokeName + " from the database...");
        ...
        connection.query(findPoke, function(err, rows, fields) {
            ...
            if (err) {
                ...
                if (rows.length > 0) {
                    // let console know
                    console.log(pokeName + " has been found. Awaiting user confirmation.");
                            ...
                            {max: 1, time: 30000}).then(collected => {
                                if (collected.first().emoji.name == '✅') {
                                    ...
                                    connection.query(deletePokeStatement, function(err, results) {
                                        if (err) {
                                            console.log("Unable to properly delete " + pokeName);
                                            message.reply("...But " + pokeName + " came back!\n((The Pokemon could not be deleted))");
                                            throw err;
                                        } else {
                                            console.log(pokeName + " has been deleted successfully.");
                                        }
                                    });
                                }
                                else {
                    ...
                    console.log("Pokemon with name " + pokeName + " not found.");
                    message.reply(
