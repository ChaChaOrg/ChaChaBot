...
exports.run = (client, connection, message, args) => {
	...
	if (args[0].includes('help')) {
		...
		message.channel.send("A Pokémon that has never evolved gains a +5 bonus to this check, and a Pokémon that has evolved once gains a +2 bonus, since it is easier to learn moves at earlier stages. However, this only applies if the evolutions can learn the same moves. If a Pokémon is a middle stage and its previous stage does not learn the move, it still gains a +5 bonus. The same applies if it is fully evolved. If it is in a final stage and only the middle stage can learn the moves, it gains a +2 bonus but not the +5. If it is in its final stage and neither previous stage learned the move, the +5 bonus is still retained. This bonus also applies when a Pokémon is trying to use a move in battle, as do all other bonuses except the change in ability modifier and user of the move.").catch(console.error);
		return;
		...
		if (successes < 3) {
			...
			message.channel.send(`The DC for ${pokeName} to learn ${moveName} is ${finalDC}.\n\nWhen rolling, use a Train Pokemon check, but utilizing the Pokemon's INT mod instead of your CHA mod. This check must be attempted while in a battle.`);
		}
