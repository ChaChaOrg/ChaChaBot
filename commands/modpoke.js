...
*/
        ...
        // check if trying to change name to an existing pokemon

        let sql = `SELECT * FROM pokemon WHERE name = '${valString}';`;
        logger.info(`[modpoke] SQL query: ${sql}`);
        let found = false;
        //console.log(sql);
        connection.query(sql, function (err, response) {
            ...
            if (response.length != 0) {
                logger.info("[modpoke] Pokemon with name already exists, exiting.")
