...
Statblock.prototype.calculateSaves = function(pokemon) {
    ...
    this.fortSave = Math.floor(.5 * pokemon.level + fortTypeBonus) + modGen(this.conBase);
    this.refSave = Math.floor(.5 * pokemon.level + refTypeBonus) + modGen(this.dexBase);
    this.willSave = Math.floor(.5 * pokemon.level + willTypeBonus) + modGen(this.wisBase);
...
//go through base formula for stat creation
    for (let ii = 1; ii < STAT_ARRAY_MAX; ii++) {
        this.formStats[ii] = Math.floor((((BASE_STAT_MULTIPLIER * this.baseStats[ii] + this.ivStats[ii] + (this.evStats[ii] / EV_MULTIPLIER)) * pokemon.level) / FORM_DIVISOR) + FORM_SHIFT);
